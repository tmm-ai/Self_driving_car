...
"""" All code in this module was provided by Picar/SunFounder, and we only made very limited adjustments"""""
...
def get_distance_at(angle):
    ...
    time.sleep(0.04)
    distance = us.get_distance() # in ultrasonic line 22
...
def get_status_at(angle, ref1=35, ref2=10):
    ...
    if dist > ref1 or dist == -2:
        return 2  # this is more than 35 cm away or too far away.
    elif dist > ref2:
        return 1  # more than 10, but less than 35 (should turn)
    else:
        return 0  # this is less than 10cm

def scan_step(ref):
    global scan_list, current_angle, us_step  # us_step ==90 or 60
    current_angle += us_step  # this adjusts current angle?? w
    if current_angle >= max_angle:
        current_angle = max_angle
        us_step = -STEP  # then we just eliminate the 90??
    elif current_angle <= min_angle:
        current_angle = min_angle
        us_step = STEP
    status = get_status_at(current_angle, ref1=ref) #ref1

    scan_list.append(status)  # list of status (2,1,0) at current angle with passed cm based on distance away.
